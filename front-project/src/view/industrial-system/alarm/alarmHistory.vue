    ...
    async getAlarmHistory() {
      try {
        ...
        for (let i = 0, length = dataLists.data.msg.length; i < length; i++) {
          this.tableData.push({
            alarmCode:
              dataLists.data.msg[i].alarmCode === null
                ? ""
                : dataLists.data.msg[i].alarmCode,
            alarmCodeName:
              dataLists.data.msg[i].alarmCodeName === null
                ? ""
                : dataLists.data.msg[i].alarmCodeName,
            alarmSolutonName:
              dataLists.data.msg[i].alarmSolutonId === "None"
                ? ""
                : dataLists.data.msg[i].alarmSolutonId,
            callbackMsg:
              dataLists.data.msg[i].callbackMsg === null
                ? ""
                : dataLists.data.msg[i].callbackMsg,
            processTime: stampToDate(dataLists.data.msg[i].processTime, "year"),
            machineName:
              dataLists.data.msg[i].machineName === null
                ? ""
                : dataLists.data.msg[i].machineName
          });
        }
      } catch (err) {
