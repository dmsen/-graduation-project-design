...
export default {
  ...
  methods: {
    ...
    async getMacTypes() {
      try {
        ...
        for (let i = 0, length = macTypeList.data.msg.length; i < length; i++) {
          this.tableData.push({
            code: macTypeList.data.msg[i].machTypeCode,
    ...
    showChangeMacType(param) {
      ...
      const { machTypeId, detail, name, code } = this.tableData[param.index];
    ...
    async handleSubmit() {
      const _this = this;
      const { isAdd, id, detail, name, code } = this.formItem;
      try {
        if (isAdd) {
